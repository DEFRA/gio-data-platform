...
az repos pr create --auto-complete false --bypass-policy false --delete-source-branch true --description 'Extracted latest Changes from Purview $AccountName' --source-branch $branch --squash true --target-branch main --title "PR for Purview Config" --project $AdoProject --org $AdoAccountUrl --repository "GIO_DATA_PLATFORM"